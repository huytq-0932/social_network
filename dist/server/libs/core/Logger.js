...
const Logger = (folderName, options = {}) => {
    ...
    const logger = winston.createLogger(Object.assign({ level: 'info', 
        //format: winston.format.json(),
        format: combine(timestamp(), splat(), 
        //prettyPrint(),
        myFormat), transports: [
            new (winston.transports.DailyRotateFile)(Object.assign({ filename: `%DATE%-error.log`, level: 'error' }, transportOptions)),
            new (winston.transports.DailyRotateFile)(Object.assign({ filename: `%DATE%-info.log`, level: 'info' }, transportOptions)),
        ] }, options));
    return logger;
};
